...
public class ElementLinkTypeHandler implements FieldHandler {

    @Override
    public Object getValue(Object o) throws IllegalStateException {
        Element element = (Element) o;
        Element.LinkType linkType = element.getLinkType();
        if (linkType != null) {
            switch (linkType) {
                case NONE:
                    return "0";
    ...
    public void setValue(Object o, Object value) throws IllegalStateException, IllegalArgumentException {
        ...
        if (linkTypeValue.equals("0"))
            linkType = Element.LinkType.NONE;
        else if (linkTypeValue.equals("1"))
            linkType = Element.LinkType.SIMPLE_LINK;
        else if (linkTypeValue.equals("2"))
            linkType = Element.LinkType.MULTI_LINK;
        else if (linkTypeValue.equals("10"))
            linkType = Element.LinkType.REFERENCE;
        else linkType = null;

        element.setLinkType(linkType);
    }
