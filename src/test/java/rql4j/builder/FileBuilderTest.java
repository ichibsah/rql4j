...
public class FileBuilderTest extends TestCase {
    ...
    public FileBuilderTest(String name) {
        super(name);
    }
    public static Test suite() {
        return new TestSuite(FileBuilderTest.class);
    }

    public void testFileBuilderSaveMedia() {
        FileBuilder fileBuilder = new FileBuilder.SaveMedia(properties.getProperty("cms.test.asset.folder.guid")).File("loading.gif", "C:\\").build();
        command.addCommand(fileBuilder);
        command.execute();
    }

    public void testFileBuilderCheck() {
        FileBuilder fileBuilder = new FileBuilder.Check(properties.getProperty("cms.test.asset.folder.guid")).File("loading.gif").build();
        command.addCommand(fileBuilder);
        Boolean fileExists = command.getResult(fileBuilder).isFileExists();
