...
public class TemplatesBuilderTest extends TestCase {
    RqlCommand command;
    ...
    public void testTemplatesBuilderList() throws Exception {
        TemplatesBuilder builder = new TemplatesBuilder.List().FolderGuid(properties.getProperty("cms.test.contentclass.folder.guid")).build();
        List<Template> templateList = command.getResult(builder).getTemplates();
        assertEquals(4, templateList.size());
        Template basis = command.getResult(builder).getTemplateByName("bas_space");
        assertEquals("bas_space", basis.getName());
    }
}
