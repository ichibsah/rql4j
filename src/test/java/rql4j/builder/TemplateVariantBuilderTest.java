...
public class TemplateVariantBuilderTest extends TestCase {
    RqlCommand command;
    String loginGuid;
    Properties properties;

    public TemplateVariantBuilderTest(String testName) {
        super(testName);
    }
    ...
    public static Test suite() {
        return new TestSuite(TemplateVariantBuilderTest.class);
    }

    public void testTemplateVariantBuilderLoad() {
        TemplateVariantBuilder templateVariantBuilder = new TemplateVariantBuilder.Load(properties.getProperty("cms.test.contentclass.template.guid")).build();
        IoData ioData = command.getResult(templateVariantBuilder);
        String code = ioData.getTemplate().getTemplateVariant().getCode();
        String codeNeu = code.replaceAll("<%img_visual_nosunlight%>", "<%img_visual_supersunlight%>");
