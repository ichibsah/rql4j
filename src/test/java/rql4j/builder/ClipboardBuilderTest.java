...
public class ClipboardBuilderTest extends TestCase {
    ...
    public ClipboardBuilderTest(String name) {
        super(name);
    }

    public void testClipboardBuilderAddPagesToLink() throws Exception {
        ClipboardBuilder clipboardBuilder = new ClipboardBuilder.AddPagesToLink(properties.getProperty("cms.test.link.guid"))
                .Entry(properties.getProperty("cms.test.page.guid")).build();
        Assert.assertEquals("ok", command.getResult(clipboardBuilder).getResult().trim());
    }
}
