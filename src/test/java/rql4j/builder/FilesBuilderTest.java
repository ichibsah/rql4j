...
public class FilesBuilderTest extends TestCase {
    RqlCommand command;
    String loginGuid;
    Properties properties;


    public FilesBuilderTest(String name) {
        super(name);
    }
    ...
    public static Test suite() {
        return new TestSuite(FilesBuilderTest.class);
    }

    public void testFilesBuilderListMedia() {
        FilesBuilder filesBuilder = new FilesBuilder.ListMedia(properties.getProperty("cms.test.asset.folder.guid"), 2000).build();
        command.addCommand(filesBuilder);
        command.execute();
        List<File> fileList = command.getResult(filesBuilder).getIoData().getFiles().getFileList();

    }

    public void testFilesBuilderDeleteFilesMedia() {
        FilesBuilder filesBuilder = new FilesBuilder.DeleteFilesMedia(properties.getProperty("cms.asset.folder.guid")).File("bmw1.png").build();
        command.addCommand(filesBuilder);
        command.execute();
    }
