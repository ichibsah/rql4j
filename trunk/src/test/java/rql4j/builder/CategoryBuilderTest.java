...
public class CategoryBuilderTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        CategoryBuilder categoryBuilder = new CategoryBuilder.AddNew("Test Category").build();
        IoData ioData = command.getResult(categoryBuilder);
        Assert.assertEquals("Test Category", ioData.getCategory().getValue());
    ...
    public CategoryBuilderTest(String name) {
        super(name);
    }

    public static Test suite() {
        return new TestSuite(CategoryBuilderTest.class);
    }

    public void testCategoryBuilderLoad() throws Exception {
        CategoryBuilder categoryBuilder = new CategoryBuilder.Load(this.categoryGuid).build();
        IoData ioData = command.getResult(categoryBuilder);
        Assert.assertEquals(this.categoryGuid, ioData.getCategory().getGuid());
    }

    public void testCategoryBuilderSave() throws Exception {
        CategoryBuilder categoryBuilder = new CategoryBuilder.Save("New Name",this.categoryGuid).build();
        IoData ioData = command.getResult(categoryBuilder);
        Assert.assertEquals("New Name", ioData.getCategory().getValue());
    }

    public void testCategoryBuilderDelete() throws Exception {
