...
public class LinkBuilderTest extends TestCase {
    ...
    public void testLinkBuilderReference() throws Exception {
        LinkBuilder linkBuilder = new LinkBuilder.Reference(properties.getProperty("cms.test.link.guid"), properties.getProperty("cms.test.page.guid")).build();
        command.getResult(linkBuilder);
    }

    public void testLinkBuilderSave() throws Exception {
        PagesBuilder pagesBuilder = new PagesBuilder.List(properties.getProperty("cms.test.link.guid")).build();
        List<Page> pageList = command.getResult(pagesBuilder).getPageList();
        Page firstElement = pageList.get(0);
        pageList.remove(firstElement);
        pageList.add(firstElement);
        LinkBuilder linkBuilder = new LinkBuilder.Save(properties.getProperty("cms.test.link.guid"), pageList).build();
        command.getResult(linkBuilder);
    }
}
