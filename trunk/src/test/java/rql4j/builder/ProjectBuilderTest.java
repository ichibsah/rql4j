...
public class ProjectBuilderTest extends TestCase {
    ...
    public ProjectBuilderTest(String name) {
        super(name);
    }

    public static Test suite() {
        return new TestSuite(ProjectBuilderTest.class);
    }

    public void testProjectBuilderLoad() {
        ProjectBuilder projectBuilder = new ProjectBuilder.Load(properties.getProperty("cms.test.project.guid")).build();
        Project project = command.getResult(projectBuilder).getProject();
        assertEquals("Best-Practice 10.1.0.1", project.getName());
    }
