...
public class LanguageVariantsBuilderTest extends TestCase {
    RqlCommand command;
    ...
    public void testLanguageVariantsBuilderList() throws Exception {
        LanguageVariantsBuilder builder = new LanguageVariantsBuilder.List().build();
        Languagevariant mainLanguage = command.getResult(builder).getMainLanguage();
        assertEquals("German", mainLanguage.getName());
        mainLanguage = command.getResult(builder).getLanguageVariantByName("German");
        assertEquals("German", mainLanguage.getName());
        List<Languagevariant> languagevariantList = command.getResult(builder).getLanguageVariants();
        assertEquals(1, languagevariantList.size());
    }
}
