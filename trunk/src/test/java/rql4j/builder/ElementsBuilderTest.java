...
public class ElementsBuilderTest extends TestCase {

    RqlCommand command;
    Properties properties;
    String loginGuid;

    public ElementsBuilderTest(String name) {
        super(name);
    }
    ...
    public static Test suite() {
        return new TestSuite(ElementsBuilderTest.class);
    }

    public void testElementBuilderLoad() {
        ElementsBuilder elementsBuilder = new ElementsBuilder.Load(properties.getProperty("cms.test.page.guid")).build();
        command.addCommand(elementsBuilder);
        command.execute();
        String guid = command.getResult(elementsBuilder)
                .getElementByName("hdlHeadline")
                .getGuid();

        assertEquals(properties.getProperty("cms.test.element.guid"), guid);
    }
}
